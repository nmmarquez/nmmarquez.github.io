...
download_smash_data <- function(){
    ...
    df$Time.end.in.secs <- to_seconds(df$Time.end)
    df <- subset(df, !is.na(Game))
    return(df)
}
...
subset_matchup <- function(df, char1, char2=""){
    ...
    #       character string to be used for subsetting
    # :return: df
    #       data frame subsetted by a particular matchup
...
subset_player <- function(df, player){
    # Return matches involving a particular player
    #
    # :param df: data frame
    #       data frame to subset
    # :param char: player
    #       player to subset with
    # :return: df
    #       data frame subsetted by a particular player
    subset(df, Player.1 == player | Player.2 == player)
} 
